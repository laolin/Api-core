...
$options = ["para1" => $para1, "para2" => $para2];
...
class Request {
   ...
   * 构造函数，保存参数
   */
  public function __construct($api, $call, $options = []) {
   ...
   */
  public function getJson($namespace, $callback = false) {
    $apiFile = $this->_getApiFile();
    if (!$apiFile) {
   ...
   * 增加调试数据到 json 数据
   */
  public static function debugJson($json) {
   ...
   * 根据构造的参数，获取需要调用的文件名
   */
  protected function _getApiFile() {
   ...
   * 安全的请求参数
   */
  public function safeQuery($name, $defaultValue = '') {
