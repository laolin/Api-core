...
class Configs {
  ...
  public static function readConfigOnce($fileName = 'config.inc.php', $deep = 5, $path = '') {
    // 保证只引用一次，即使在多个目录中存在
  ...
  public static function readConfig($fileName = 'config.inc.php', $deep = 5, $path = '', $onlyonce = false) {
    if (!$path) {
...
class API {
  ...
  public static function enable_debug($b) {
    self::$enable_debug = $b;
  }
  public static function debug($value, $k = '') {
    if ($k) {
      self::$debug[$k] = $value;
    } else {
      self::$debug[] = $value;
  ...
  public static function cn_json($arr) {
    return json_encode($arr, JSON_UNESCAPED_UNICODE);
  }

  public static function toJson($res) {
    if (is_array($res)) {
      $json = ['errcode' => $res[0]];
      if ($res[0]) {
        $json['errmsg'] = $res[1];
   ...
   */
  public static function call($module, $api, $param) {
   ...
   * 方式：curl, post
   */
  public static function post($module, $api, $param) {
    $url = $module . $api;
...
class FN {
  ...
  public static function array_column($arr, $k1, $k2 = false) {
    if ($k2 === true) {
      $R = [];
      foreach ($arr as $row) {
        $R[$row[$k1]] = $row;
