...
class QgsModules {
  ...
  public static function DB() {
    if (!self::$db) {
      self::$db = new mymedoo(self::database_name);
      self::refreshWxUser();
    }
    return self::$db;
  }
  ...
  /** 向独立服务器同步微信用户数据 */
  public static function refreshWxUser() {
    $db = self::DB();
    $timeupdate = $db->max(self::table_wxuser, 'timeupdate');
    \DJApi\API::debug(["上次更新时间" => $timeupdate, 'DB' => $db->getShow()]);
    $json = \DJApi\API::post(SERVER_API_ROOT, "user/wx/sync", ['timeupdate' => $timeupdate]);

    \DJApi\API::debug(['请求同步微信', '上次更新时间' => $timeupdate, 'json' => $json]);

    if (!\DJApi\API::isOK($json)) {
      return;
    }

    $wxUser_renew = $json['datas']['list'];
    \DJApi\API::debug(['wxUser_renew' => $wxUser_renew]);
    // 保存这这些数据, 包括未关注的
    if (count($wxUser_renew)) {
      $save_datas = array_map(function ($row) {
        $newRow = [];
        $fields = ['timeupdate', "subscribe", "openid", "nickname", "sex", "language", "city", "province", "country", "headimgurl", "subscribe_time", "unionid", "remark", "groupid"];
        foreach ($fields as $k) {
          $newRow[$k] = isset($row[$k]) ? $row[$k] : '';
        }
        $newRow['isgz'] = $newRow['subscribe'] == '1' ? "已关注" : "未关注";
        return $newRow;
      }, $wxUser_renew);
      // 以 openid 为依据进行更新
      $openids_all = array_map(function ($row) {return $row['openid'];}, $save_datas);
      $openid_old = $db->get(self::table_wxuser, ['openid'], ['openid' => $openids_all]);
      \DJApi\API::debug(["原有微信数据" => $openid_old, 'DB' => $db->getShow()]);
      $new_datas = [];
      $n = 0;
      foreach ($save_datas as $row) {
        if (in_array($row['openid'], $openid_old)) {
          $n += $db->update(self::table_wxuser, $row, ["openid" => $row['openid']]);
        } else {
          $new_datas[] = $row;
        }
      }
      \DJApi\API::debug(["更新了微信 $n 行", $db->getShow()]);
      $n = 0;
      // 新增的行
      if (count($new_datas) > 0) {
        $n = $db->insert(self::table_wxuser, $new_datas);
        \DJApi\API::debug(['插入数据库', 'new_datas' => $new_datas, 'n' => $n, 'DB' => $db->getShow()]);
      }
    }
...
function ajax_get_userinfo($db, $query) {
  if (!isset($query['t']) || !isset($query['sign']) || !isset($query['module'])) {
    json_error(1000, "参数缺失");
  }

  if (!isset($query['uid']) && !isset($query['nick'])) {
    json_error(1000, "参数缺失");
  }
  ...
  $where = ["AND" => []];
  if (isset($query['uid']) && $query['uid'] + 0 > 0) {
    $where["AND"]["id"] = $query['uid'];
  } else {
    $where["AND"]["OR"] = ["nick" => $query['nick'], "mobile" => $query['nick'], "id" => $query['nick']];
  }
...
function ASSERT_uid(&$db, &$query, $module = false, $where = false) {
  ...
  if (!is_array($userinfo)) {
    if ($GLOBALS['require_id'] > 0) {
      $db->update("log_api", ["result" => "1002, 用户ID错误"], ["id" => $GLOBALS['require_id']]);
    }
  ...
  if (in_array($module, ["user", "expert", "service"]) && $userinfo[$module . 'state'] < 3) {
    if ($GLOBALS['require_id'] > 0) {
      $db->update("log_api", ["result" => "1011, 用户未激活"], ["id" => $GLOBALS['require_id']]);
    }
  ...
  if (!check_password_sign($t, $password, $query['sign']) &&
    ...
    if ($GLOBALS['require_id'] > 0) {
      $db->update("log_api", ["result" => "1003, 密码错误"], ["id" => $GLOBALS['require_id']]);
    }
...
function ASSERT_right(&$db, &$userinfo, $needrights = false) {
  ...
  if ($needrights) {
    ...
    if (in_array("超级管理员", $userinfo['d']['权限'])) {
      return;
    }

    if (is_string($needrights)) {
      $needrights = [$needrights];
    }

    foreach ($needrights as $myright) {
      if (in_array("$myright", $userinfo['d']['权限'])) {
        continue;
      }
...
function ParseAttr(&$row, $field) {
  if (!isset($row['attr']) || is_array($row[$field])) {
    return;
  }
...
function ASSERT_query_userinfo(&$db, &$query, $fields, $check_module = false) {
  ...
  foreach ($base_fields as $k) {
    if (!isset($query[$k])) {
      echo json_error(1000, "参数缺失: $k", [$query]);
      exit;
    }
  }
...
function append_wx_info(&$db, $arr_user, $fileds = false) {
  if (!$fileds) {
    $fields = ["headimgurl", "nickname"];
  }

  $openids = [];
  foreach ($arr_user as $k => $row) {
    $openids[$row['openid']] = $k;
  }
...
function get_page_data(&$db, $query, $table, $AND, $fileds = "*", $ORDER = false) {
  $page = $query['page'] - 1;if ($page < 0) {
    $page = 0;
  }

  $pagesize = $query['pagesize'] + 0;if ($pagesize < 1) {
    $pagesize = 1;
  }
